...
contract Banks {
    ...
    event BankAdded(address id_, string name, string email);
    ...
    event BankCustomAttributeAdd(Types.CustomAttribute customAttribute);
...
function AddCustomAttribute(Types.CustomAttribute memory CUSTOM) internal{
      
      emit BankCustomAttributeAdd(CUSTOM);
}
    // Contract Methods
        ...
        returns (uint256 totalPages, Types.Bank[] memory)
    {
        ...
        (
            ...
            uint256 startIndex_,
            uint256 endIndex_
        ) = Helper.getIndexes(pageNumber, bankList);
    ...
    function addbank(Types.Bank memory bank_) internal {
        ...
        emit BankAdded(bank_.id_, bank_.name, bank_.email);
    }
